...
raw_dev_data = read_raw_data('data/mul_class_86_dev_data.txt')
...
from my_circuit_hadamard_masking import IQPAnsatz
N = AtomicType.NOUN
...
for index, diagram in enumerate(uncuped_train_diagrams):
    ansatz = IQPAnsatz(masked_token = training_tokens[index],ob_map={N: 1, S: 4, P:1}, n_layers=4, n_single_qubit_params=8)
    train_circuits.append(ansatz(diagram))
for index, diagram in enumerate(uncuped_dev_diagrams):
    ansatz = IQPAnsatz(masked_token = dev_tokens[index],ob_map={N: 1, S: 4, P:1}, n_layers=4, n_single_qubit_params=8)
